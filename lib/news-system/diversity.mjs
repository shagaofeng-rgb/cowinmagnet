...
const SEMANTIC_REJECT_THRESHOLD = 0.85;
const MIN_INFORMATION_GAIN_SCORE = 5;
...
function sourceDomain(item) {
  return rootDomainFromUrl(item.url || item.canonicalUrl || item.sourceUrl || "");
}

function feedDomain(item) {
  return rootDomainFromUrl(item.sourceUrl || item.url || "");
}
...
export function topicClusterId(item = {}) {
  ...
  if (/cement|aggregate|quarry|limestone|crusher|concrete/.test(text)) return "cement-aggregate-iron-removal";
  if (/food|powder|granule|foreign material|contamination/.test(text)) return "food-metal-contamination-control";
  if (/battery|lithium|black mass|brine|critical mineral|rare earth/.test(text)) return "critical-minerals-battery-materials";
  if (/recycling|scrap|waste|plastic|sorting|mrf/.test(text)) return "recycling-application";
...
export function evaluateNewsDiversity(item, context, selected = []) {
  ...
  const sameSourcePenalty = usedSource72h || lastTenSourceCount >= 2 ? 3 : 0;
  ...
  if (lastTenSourceCount >= 2) rejectedReasons.push("source-domain-limit-last-10");
  if (semantic.score > SEMANTIC_REJECT_THRESHOLD) rejectedReasons.push("semantic-duplication-over-0.85");
  if (topic24hCount >= 1) rejectedReasons.push("topic-cluster-24h-limit");
  if (topic7dCount >= 3) rejectedReasons.push("topic-cluster-7d-limit");
