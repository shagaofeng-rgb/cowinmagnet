...
import { hasNewsPublishingRelevance, scoreNewsItem, passesScoreThreshold } from "./scoring.mjs";
...
function isAllowedItem(item) {
  ...
  // A source may cover a target industry without covering magnetic separation.
  // Require both an equipment/control signal and a concrete application context
  // before it can enter the scoring and publishing pipeline.
  if (!hasNewsPublishingRelevance(item)) return false;

  if (item.sourceName === "Food Safety News") {
    return /metal contamination|foreign material|magnetic|magnet|processing equipment|inspection system/.test(text);
  }
...
export function recentNoImageContext(runs = []) {
  const urls = new Set();
  const failureCountsByDomain = new Map();
  const retryWindowMs = Number(process.env.NEWS_NO_IMAGE_RETRY_WINDOW_MS || 6 * 60 * 60 * 1000);
  const domainFailureThreshold = Math.max(1, Number(process.env.NEWS_NO_IMAGE_DOMAIN_FAILURE_THRESHOLD || 2));
  const now = Date.now();
  for (const run of runs || []) {
    ...
    for (const item of run?.items || []) {
      ...
      if (url) urls.add(url);
      const domain = rootDomainFromUrl(item?.publisherUrl || item?.resolvedSourceUrl || item?.url || item?.sourceUrl || "");
      if (domain) failureCountsByDomain.set(domain, (failureCountsByDomain.get(domain) || 0) + 1);
    }
  }
  return {
    urls,
    domains: new Set([...failureCountsByDomain.entries()].filter(([, count]) => count >= domainFailureThreshold).map(([domain]) => domain)),
...
function selectCandidates({
  ...
  state,
  limit,
  automationIndex = new Map(),
  mode = newsSystemConfig.publishMode,
  noImageUrls = new Set(),
  noImageDomains = new Set()
}) {
  const dedupeReasons = {};
  ...
  // Automated publishing must always keep the verified source-pool safeguards enabled.
  const strictSourcePool = true;
  const requiredGroups = strictSourcePool ? newsSystemConfig.sourcePool?.requiredGroupsPerRun || 3 : 1;
  const requiredNewDomains = strictSourcePool ? newsSystemConfig.sourcePool?.requiredNewDomainsPerRun || 2 : 0;
  ...
  for (const item of scored) {
    const skipRecentNoImageFailures = process.env.NEWS_SKIP_RECENT_NO_IMAGE === "true";
    if (skipRecentNoImageFailures && noImageUrls.has(normalizeUrl(item.url || item.canonicalUrl || ""))) {
    ...
    const itemDomain = rootDomainFromUrl(item.publisherUrl || item.resolvedSourceUrl || item.url || item.sourceUrl || "");
    if (skipRecentNoImageFailures && itemDomain && noImageDomains.has(itemDomain)) {
      dedupeReasons["skipped-no-source-image-domain-recent"] =
        (dedupeReasons["skipped-no-source-image-domain-recent"] || 0) + 1;
      rejectedSources.push(compactRejectedSource(item, item.diversity, "skipped-no-source-image-domain-recent"));
      continue;
    }
...
async function runNewsAutomationJobUnlocked({
  ...
  const noImageContext = recentNoImageContext(recentRuns);
  ...
  const { candidates, scoredCount, dedupeReasons, diversityLog } = selectCandidates({
    ...
    automationIndex,
    mode,
    noImageUrls: noImageContext.urls,
    noImageDomains: noImageContext.domains
  });
  ...
  for (const candidate of candidates) {
    ...
    const requireSourceImage = process.env.NEWS_REQUIRE_SOURCE_IMAGE !== "false";
    if (requireSourceImage && (imagePlan.sourceImage?.imageStatus !== "valid" || !imagePlan.coverImage?.imageUrl)) {
      ...
      items.push({
        ...
        workflow: {
          ...
          history: [{ status: "skipped_no_source_image", at: new Date().toISOString(), by: "system" }]
        }
      });
      continue;
    }
