  ...
  const fetchedDomains = [
    ...new Set(
      fetchedItems
        .map((item) => {
          try {
            return new URL(item.url || item.sourceUrl || "").hostname.replace(/^www\./, "");
          } catch {
            return "";
          }
        })
    ...
    return {
      ...
      diversityLog: {
        ...
        rules: {
          ...
          same_domain_recent_10_limit: 2,
          semantic_reject_threshold: 0.85,
          topic_24h_limit: 1,
          topic_7d_limit: 3,
          minimum_information_gain_score: 5
        }
      }
    };
  }
...
async function runNewsAutomationJobUnlocked({
  ...
  const { candidates, scoredCount, dedupeReasons, diversityLog } = selectCandidates({ fetchedItems, state, limit, automationIndex, mode });
