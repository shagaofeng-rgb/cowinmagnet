...
async function runNewsAutomationJobUnlocked({
  ...
  for (const item of candidates) {
    ...
    const companyImagePlan = await matchCompanyImages({
      item,
      article: generated,
      productMatch,
      minInlineImages: newsSystemConfig.imagePolicy.minInlineImages,
      maxInlineImages: newsSystemConfig.imagePolicy.maxInlineImages
    });
    const displayCover = companyImagePlan.cover || imagePlan.coverImage;
    const articleBase = {
      ...generated,
      scores: item.scores,
    ...
    const cover = displayCover
      ? {
          coverImage: displayCover.imageUrl,
          coverAlt: displayCover.imageAlt,
          imageCaption: displayCover.imageCaption,
          imageSourceName: displayCover.imageSourceName,
          imageSourceUrl: displayCover.imageSourceUrl,
          imageLicenseNote: displayCover.imageAttributionText,
          sourceImage: imagePlan.sourceImage,
          generatedCoverPath: "",
    ...
    const generatedArticle = {
      ...
      bodyImages: companyImagePlan.inlineImages || []
    };
