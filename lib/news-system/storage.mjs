...
const NEWS_SCHEMA_LOCK_ID = 52402000;
...
async function ensureNewsSchema() {
  ...
  try {
    await client.query("SELECT pg_advisory_lock($1)", [NEWS_SCHEMA_LOCK_ID]);
    await client.query(`
    ...
    `);
    await client.query("CREATE INDEX IF NOT EXISTS news_job_runs_date_idx ON news_job_runs (run_date DESC, started_at DESC)");
    await client.query("CREATE INDEX IF NOT EXISTS news_job_runs_status_idx ON news_job_runs (status)");
    schemaReady = true;
  } finally {
    await client.query("SELECT pg_advisory_unlock($1)", [NEWS_SCHEMA_LOCK_ID]).catch(() => {});
    client.release();
  }
}
