...
const AGGREGATOR_IMAGE_HOSTS = new Set(["googleusercontent.com", "gstatic.com"]);
...
function firstJsonLdImages(html = "") {
  ...
  function collect(value, imageContext = false) {
    if (!value) return;
    if (typeof value === "string") {
      if (imageContext) images.push(value);
    } else if (Array.isArray(value)) value.forEach((entry) => collect(entry, imageContext));
    else if (typeof value === "object") {
      if (imageContext && value.url) images.push(value.url);
      if (imageContext && value.contentUrl) images.push(value.contentUrl);
      if (value.image) collect(value.image, true);
      if (value.thumbnailUrl) collect(value.thumbnailUrl, true);
      if (value["@graph"]) collect(value["@graph"], false);
    }
  }

  for (const block of blocks) {
    try {
      collect(JSON.parse(decodeHtml(block[1]).trim()), false);
    } catch {}
  }
...
function bodyImageCandidates(html = "") {
  ...
  while ((match = imgRegex.exec(source))) {
    ...
    const src =
      ...
      "";
    const srcset =
      tag.match(/\bdata-srcset=["']([^"']+)["']/i)?.[1] || tag.match(/\bsrcset=["']([^"']+)["']/i)?.[1] || "";
...
function looksLikeBadImage(url = "") {
  ...
  try {
    ...
    if ([...AGGREGATOR_IMAGE_HOSTS].some((blocked) => host === blocked || host.endsWith(`.${blocked}`))) return true;
  } catch {}
  return BAD_IMAGE_TERMS.some((term) => lower.includes(term));
}

export function isApprovedSourceImageUrl(url = "") {
  return isSafeHttpUrl(url) && !looksLikeBadImage(url);
}
...
async function validateImage(candidate, sourcePageUrl) {
  ...
  if (!isApprovedSourceImageUrl(imageUrl)) return { ok: false, reason: "logo-ad-or-svg", imageUrl };
...
export async function buildImagePlan(item, productMatch, article = null) {
  ...
  const suppliedCandidate = { url: item?.imageUrl || "", kind: "source-thumbnail", alt: item?.title || "" };
  const candidates = (item?.preferProvidedImage ? [suppliedCandidate, ...pageCandidates] : [...pageCandidates, suppliedCandidate]).filter(
  ...
  for (const candidate of uniqueCandidates) {
    ...
    const imageRecord = {
      ...
      imageAttributionText: `Image source: ${sourceName}.`,
