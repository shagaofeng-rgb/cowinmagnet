...
function normalizeStatus({ requestedStatus, quality }) {
  // This module is used by the automated collector only. Human publication is
  // handled by the authenticated CMS endpoint, after editorial review.
  return "draft";
}
...
export async function publishGeneratedArticle({ article, cover, item, quality, publishMode = newsSystemConfig.publishMode }) {
  ...
  const cmsItem = await saveCmsItem({
    ...
    sourceUrl: item.url || "",
    sourcePublisher: item.sourceName || article.sources?.[0]?.name || "",
    relevanceStatus: quality.publishable ? "pending-editorial-review" : "rejected-by-quality-check",
    duplicateFingerprint: item.fingerprints?.combined || item.fingerprints?.title || "",
