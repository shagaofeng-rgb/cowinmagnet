...
import { createArticleDocumentFromLegacy, normalizeArticleDocument, publicArticlePayload } from "./articleDocument.js";
...
export function textToSections(value = "") {
  const content = sanitizeArticleContent(value);
  const blocks = content.split(/\n\s*\n/).map((block) => block.trim()).filter(Boolean);
  // Legacy admin callers still receive sections, but orphan paragraphs are never promoted to fake headings.
  return blocks.length ? [{ heading: "Overview", body: blocks.join("\n\n") }] : [];
}
...
export async function saveCmsItem(item) {
  ...
  const privateProductFields = item.type === "product"
    ...
    : {};
  const articleDocument = ["news", "blog"].includes(item.type)
    ? publicArticlePayload(item.articleDocument ? normalizeArticleDocument(item.articleDocument) : createArticleDocumentFromLegacy(item))
    : undefined;
  const privateArticleFields = ["news", "blog"].includes(item.type)
    ? { prompt: undefined, sourceClaims: undefined, internalLinkPlan: undefined, seoFields: undefined, cmsChecklist: undefined, evidenceFiles: undefined, authoringNotes: undefined, llmCommentary: undefined, qaNotes: undefined, reviewNotes: undefined, workflow: undefined }
    : {};
  const normalized = {
    ...item,
    ...privateProductFields,
    ...privateArticleFields,
    ...(articleDocument ? {
      articleDocument,
      title: articleDocument.title || item.title,
      excerpt: articleDocument.summary || item.excerpt,
      seoTitle: articleDocument.seo.metaTitle || item.seoTitle,
      seoDescription: articleDocument.seo.metaDescription || item.seoDescription,
...
export async function updateCmsItemPublicationStatus(type, slug, { status, editorialStatus }) {
  invalidateCmsReadCache(type);
  const db = getPool();
  if (db) {
    await ensureSchema();
    await db.query(
      `
        ...
        SET payload = jsonb_set(
              ...
              '{editorialStatus}', to_jsonb($4::text), true
            ),
            updated_at = NOW()
        WHERE type = $1 AND slug = $2
      `,
      [type, slug, status, editorialStatus]
    );
    await notifySitemapChange({ type, slug }, status);
    return;
  }
  ...
  const nextItems = items.map((item) =>
    item.type === type && item.slug === slug
      ? { ...item, status, editorialStatus, updatedAt: new Date().toISOString() }
      : item
  );
  await writeFileItems(nextItems);
  await notifySitemapChange({ type, slug }, status);
