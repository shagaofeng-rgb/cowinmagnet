...
export function normalizeBlogPost(post) {
  ...
  return {
    ...
    robots: post.robots || "",
    articleDocument: post.articleDocument || null
  };
}
...
export async function getPublicBlogPostsWithCms(options = {}) {
  const posts = await getBlogPostsWithCms(options);
  return posts.filter(isIndexableBlog);
}
