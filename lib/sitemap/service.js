...
// Increment this when sitemap eligibility rules change so stored snapshots are
// regenerated instead of continuing to serve URLs excluded by a new policy.
const SITEMAP_POLICY_VERSION = "2026-09-08-en-canonical-noindex-v2";
...
export async function buildFreshSitemapSnapshot({ previousSnapshot = null, siteUrl = SITEMAP_SITE_URL, maxUrls, maxBytes } = {}) {
  ...
  return {
    ...payload,
    policyVersion: SITEMAP_POLICY_VERSION,
...
export async function runSitemapMaintenance({
  ...
  const lockResult = await withSitemapGenerationLock(async () => {
    ...
    const changed =
      ...
      previousState.snapshot.manifestHash !== snapshot.manifestHash ||
      previousState.snapshot.policyVersion !== SITEMAP_POLICY_VERSION;
    ...
    // Search Console submissions are for sitemap discovery, never individual
    // URL indexing. A privileged forced maintenance run is also allowed to
    // resubmit after an eligibility-policy change.
    if (!previewReadOnly && !dryRun && submit && (changed || force)) {
      try {
    ...
    const submissionRequired = !previewReadOnly && !dryRun && submit && (changed || force);
    const submissionFailed = submissionRequired && !submission.success;
    const finishedAt = new Date().toISOString();
    const status = previewReadOnly
      ? "preview_read_only"
      : dryRun
        ? "dry_run"
        : submissionFailed
    ...
    if (!previewReadOnly) {
      await recordRun({
        ...
        failedCount: submissionFailed ? 1 : 0,
        errorMessage: submissionFailed ? (submission.error || submission.reason || "Search Console sitemap submission failed") : "",
        metadata
      });
    }

    return {
      success: !submissionFailed,
      status,
      changed,
      saved: Boolean(saveResult.saved),
      snapshot,
...
export async function getRenderableSitemapSnapshot() {
  ...
  try {
    ...
    const needsPolicyRefresh = state.snapshot?.policyVersion !== SITEMAP_POLICY_VERSION;
    if (isUsableSnapshot(state.snapshot) && !isSitemapStateDirty(state) && !needsPolicyRefresh) return state.snapshot;
  } catch (error) {
    console.error("[sitemap] failed to read current snapshot", { message: safeError(error) });
  }

  const run = await runSitemapMaintenanceSafely({
    trigger: "public-request",
    force: !state?.snapshot || state.snapshot?.policyVersion !== SITEMAP_POLICY_VERSION,
    submit: false
  });
