...
import { generatedNewsPosts as legacyNewsPosts } from "../../data/generatedNews.js";
...
import { isIndexableBlog } from "../blogContentPolicy.js";
...
export async function collectSitemapSections({ siteUrl = SITEMAP_SITE_URL } = {}) {
  ...
  const blogs = dedupeBySlug([...cmsBlogs, ...staticBlogPosts].filter(isIndexableBlog));
  const legacyNews = legacyNewsPosts.filter((post) => post.quality?.passed !== false && isIndexableNews(post));
  const news = dedupeBySlug([...cmsNews, ...legacyNews, ...staticNewsPosts].filter(isIndexableNews));
  ...
  const publicStaticRoutes = STATIC_ROUTES.filter((route) => route !== "/blog" || blogs.length > 0);
  const pages = publicStaticRoutes.flatMap((route) => canonicalEntries(route, pageDates[route] || staticDates.pages, siteUrl));
