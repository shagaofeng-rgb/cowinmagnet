...
function getPools() {
  ...
  if (!pools) {
    pools = databaseUrlFallbacks().map((connectionString) => new Pool({
      ...
      // Try the preferred pooled endpoint briefly before using the verified
      // direct endpoint. This keeps the cron well within Vercel's duration.
      connectionTimeoutMillis: Math.min(CONNECTION_TIMEOUT_MS, 3500),
...
async function ensureSchema() {
  if (!getPools()) return;
  if (!schemaPromise) {
    schemaPromise = withDatabaseFallback(async (db) => {
      await db.query(`
...
export async function markSitemapDirty(reason = {}) {
  ...
  if (getPools()) {
    await ensureSchema();
    await withDatabaseFallback((db) => db.query(
      `
...
export async function getCurrentSitemapState() {
  if (getPools()) {
    ...
    const [snapshotResult, controlResult] = await Promise.all([
      withDatabaseFallback((db) => db.query("SELECT payload FROM sitemap_snapshots WHERE is_current = TRUE ORDER BY created_at DESC LIMIT 1")),
      withDatabaseFallback((db) => db.query("SELECT dirty_at, dirty_reason FROM sitemap_control WHERE id = 'default'"))
    ]);
    return {
...
export async function saveCurrentSitemapSnapshot(snapshot) {
  if (getPools()) {
    await ensureSchema();
    const connection = await withDatabaseFallback(async (db) => ({ db, client: await db.connect() }));
