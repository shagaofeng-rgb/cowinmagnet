...
export async function getCurrentSitemapState() {
  ...
  if (db) {
    ...
    const [snapshotResult, controlResult] = await Promise.all([
      db.query("SELECT payload FROM sitemap_snapshots WHERE is_current = TRUE ORDER BY created_at DESC LIMIT 1"),
      db.query("SELECT dirty_at, dirty_reason FROM sitemap_control WHERE id = 'default'")
    ]);
    return {
