...
export function canonicalEntries(path, lastmod, siteUrl, locales = ["en"]) {
  const localizePath = (locale, route) => route === "/" ? `/${locale}` : `/${locale}${route}`;
  const alternates = [
    ...locales.map((locale) => ({ hreflang: locale, href: `${siteUrl}${localizePath(locale, path)}` })),
    { hreflang: "x-default", href: `${siteUrl}${localizePath("en", path)}` }
  ];

  return [{ loc: `${siteUrl}${localizePath("en", path)}`, lastmod, alternates }];
}
