    ...
    schemaPromise = (async () => {
      await db.query(`
        ...
        CREATE TABLE IF NOT EXISTS news_operation_locks (
          lock_name TEXT PRIMARY KEY, owner_id TEXT NOT NULL, expires_at TIMESTAMPTZ NOT NULL,
...
export async function saveEditorialPlan(plan) {
  ...
  const result = await db.query(
    ...
    [
      plan.id || id("plan"),
      plan.scheduledFor || plan.scheduled_for || null,
      plan.industry || "",
      plan.country || "",
      plan.angle,
      plan.primaryProductId || plan.primary_product_id || "",
      json(plan.secondaryProductIds || plan.secondary_product_ids || []),
      json(plan.candidateIds || plan.candidate_ids || []),
      plan.status || "planned",
      plan.reason || "",
    ]
  );
...
export async function withNewsPublicationLock(callback) {
  ...
  const acquired = await db.query(`
    INSERT INTO news_operation_locks (lock_name, owner_id, expires_at, updated_at)
    VALUES ('news-publication', $1, NOW() + INTERVAL '3 minutes', NOW())
    ON CONFLICT (lock_name) DO UPDATE
      SET owner_id = EXCLUDED.owner_id,
          expires_at = EXCLUDED.expires_at,
          updated_at = NOW()
      WHERE news_operation_locks.expires_at < NOW()
    RETURNING owner_id
  `, [ownerId]);
  if (acquired.rows[0]?.owner_id !== ownerId) return { locked: false };
  try {
    return await callback();
  } finally {
    await db.query("DELETE FROM news_operation_locks WHERE lock_name = 'news-publication' AND owner_id = $1", [ownerId]);
  }
}
