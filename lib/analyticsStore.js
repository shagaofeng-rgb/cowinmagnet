...
export async function getAnalyticsSnapshot({ days = 14 } = {}) {
  ...
  const allEvents = await readAnalyticsEvents({ days: 365, limit: 20000 });
  ...
  const dashboard = {
    ...
    journeys: [...journeyMap.entries()]
      ...
      .sort((a, b) => b.value - a.value)
      .slice(0, 20),
    searchConsole: await getSearchConsoleSnapshot()
  };
...
export async function getSearchConsoleSnapshot() {
  ...
  try {
    const liveSnapshot = await getLiveSearchConsoleSnapshot();
