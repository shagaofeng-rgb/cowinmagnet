...
export async function getAnalyticsSnapshot(options = {}) {
  ...
  const [currentEvents, historyEvents] = await Promise.all([
    ...
    readAnalyticsEvents({ days: 731, limit: 100000 })
  ]);
  const visitorDayProfiles = buildVisitorDayProfiles(historyEvents);
  const events = currentEvents.filter((event) => inRange(event, startDate, endDate));
  const pageViewEvents = events.filter((event) => event.type === "page_view");
  const attributedPageViewEvents = pageViewEvents.map((event) => ({
    ...event,
    sourcePlatform: event.sourcePlatform || detectSourcePlatform(event),
    sourceDetail: event.sourceDetail || sourceDetail(event)
  }));
  ...
  const dashboard = {
    ...
    storageMode: isDatabaseConfigured() ? "database" : "file",
