...
import {
  appendDatabaseEvent,
  countDatabaseAnalyticsEvents,
  isDatabaseConfigured,
  readDatabaseAnalyticsSnapshot,
  readDatabaseEvents,
  readDatabaseVisitorJourney,
  readDatabaseVisitorHistory,
  writeDatabaseAnalyticsSnapshot
...
const PERSISTENT_SNAPSHOT_TTL_MS = Number(process.env.ADMIN_ANALYTICS_SNAPSHOT_TTL_MS || 30 * 60 * 1000);
const PERSISTENT_SNAPSHOT_MAX_STALE_MS = Number(process.env.ADMIN_ANALYTICS_SNAPSHOT_MAX_STALE_MS || 24 * 60 * 60 * 1000);
const ANALYTICS_SNAPSHOT_EVENT_LIMIT = Math.max(
  1000,
  Number(process.env.ADMIN_ANALYTICS_SNAPSHOT_EVENT_LIMIT || 12000)
);
const ANALYTICS_QUALITY_VERSION = "customer-events-v2";
const snapshotCache = new Map();
let lastAnalyticsDatabaseError = "";
const INTERNAL_ADMIN_COOKIE = "cowin_admin_session";
const NON_CUSTOMER_EVENT_TYPES = new Set(["outbound_link_click", "click_email", "click_phone"]);

function excludedIps() {
  return new Set(
    String(process.env.ANALYTICS_EXCLUDED_IPS || "")
      .split(",")
...
function hasExplicitTestMarker(event = {}) {
  const attribution = event.attribution?.sessionTouch || {};
  const values = [
    event.sourcePath,
    event.eventId,
    event.visitorId,
  ...
  return Boolean(event.isTest) || /\b(test|testing|monitor|automation)\b/i.test(values);
}

export function isCustomerAnalyticsEvent(event = {}) {
  if (!event || typeof event !== "object") return false;
  if (event.operatorTraffic || hasExplicitTestMarker(event)) return false;
  if (event.ip && excludedIps().has(String(event.ip))) return false;
  return true;
}

function isCustomerJourneyEvent(event = {}) {
  return isCustomerAnalyticsEvent(event) && !NON_CUSTOMER_EVENT_TYPES.has(event.type);
}
...
function publicDatabaseError(error) {
  ...
  if (/quota/i.test(message)) return "database quota exceeded";
  if (/timeout/i.test(message)) return "database timeout";
  if (!message) return "";
...
const WHATSAPP_EVENT_TYPES = new Set(["click_whatsapp", "whatsapp_click"]);
const WHATSAPP_PLACEMENTS = new Set([
  "floating",
  "header",
  "mobile-menu",
  "footer-chat",
...
function normalizeWhatsAppContext(payload, eventType) {
  if (!WHATSAPP_EVENT_TYPES.has(eventType)) return undefined;
  const raw = payload?.whatsapp && typeof payload.whatsapp === "object" ? payload.whatsapp : {};
  const placement = String(raw.placement || "other").slice(0, 60);
  return {
    status: "clicked",
    placement: WHATSAPP_PLACEMENTS.has(placement) ? placement : "other",
    component: String(raw.component || "site-link").slice(0, 80),
    productSlug: String(raw.productSlug || "").slice(0, 160),
    productName: String(raw.productName || "").slice(0, 180),
    locale: String(raw.locale || "").slice(0, 20),
    clickId: String(raw.clickId || "").slice(0, 100)
  };
}
...
export function normalizeAnalyticsEvent(payload, request) {
  ...
  const eventType = String(payload.type || "page_view").slice(0, 80);
  const event = {
    ...
    operatorTraffic: new RegExp(`(?:^|;\\s*)${INTERNAL_ADMIN_COOKIE}=`).test(getHeader(request, "cookie")),
    whatsapp: normalizeWhatsAppContext(payload, eventType),
    timestamp: now
  };
...
export async function appendAnalyticsEvent(event) {
  if (!isCustomerAnalyticsEvent(event)) {
    return { ok: true, storageMode: getAnalyticsStorageMode(), skipped: true };
  }
  if (isDatabaseConfigured()) {
    try {
      const saved = await appendDatabaseEvent(event);
      if (saved) {
        lastAnalyticsDatabaseError = "";
        return { ok: true, storageMode: "database", databaseStatus: "ok" };
      }
    } catch (error) {
      console.error("Database analytics write failed", error);
      lastAnalyticsDatabaseError = publicDatabaseError(error);
    }
  }
  ...
  return {
    ok: true,
    storageMode: isDatabaseConfigured() ? "database-unavailable-file-fallback" : getAnalyticsStorageMode(),
    databaseStatus: isDatabaseConfigured() ? "unavailable" : "not-configured",
    databaseError: lastAnalyticsDatabaseError
  };
}
...
export async function readAnalyticsEvents(options = {}) {
  if (isDatabaseConfigured()) {
    try {
      ...
      lastAnalyticsDatabaseError = "";
      return (events || []).filter(isCustomerAnalyticsEvent);
    } catch (error) {
...
async function readVisitorHistoryEvents(options = {}) {
  const scopedVisitorIds = Array.isArray(options.visitorIds)
    ? new Set(options.visitorIds.filter(Boolean).map(String))
    : null;
  ...
  return events
    ...
    .filter((event) => !scopedVisitorIds || scopedVisitorIds.has(String(event.visitorId)))
    .map((event) => ({
      type: "page_view",
...
function buildCustomerProfiles(events, visitorDayProfiles, visitorNumbers) {
  const customers = new Map();

  events.forEach((event) => {
    if (!event.visitorId) return;
    const existing = customers.get(event.visitorId);
    const profile = getVisitorProfile(event, visitorDayProfiles);
    const eventTime = new Date(event.timestamp).getTime() || 0;
    ...
    record.visitCount += 1;
    if (event.sessionId) record.sessionIds.add(event.sessionId);
    if (event.page) record.pages.add(event.page);
    if (eventTime < (new Date(record.firstSeenAt).getTime() || eventTime)) record.firstSeenAt = event.timestamp;
    if (eventTime >= (new Date(record.lastSeenAt).getTime() || 0)) {
      record.lastSeenAt = event.timestamp;
      record.latest = event;
      record.profile = profile;
    }
    customers.set(event.visitorId, record);
  });
  ...
  return [...customers.values()]
    ...
    .map((record) => ({
      ...
      country: record.latest.country,
      city: record.latest.city,
      device: record.latest.device,
      browser: record.latest.browser,
      channel: record.latest.channel,
      sourcePlatform: record.latest.sourcePlatform,
      sourceDetail: record.latest.sourceDetail,
      latestPage: record.latest.page,
      visitDate: record.profile.visitDate,
      visitDayNumber: record.profile.visitDayNumber,
      customerType: record.profile.customerType,
      customerTypeLabel: record.profile.customerTypeLabel
    }));
}
...
function countBySelector(events, selector, limit = 12) {
  const map = new Map();
  events.forEach((event) => {
    const value = selector(event) || "Unknown";
    map.set(value, (map.get(value) || 0) + 1);
  });
  return [...map.entries()]
    .map(([label, value]) => ({ label, value }))
...
export function buildWhatsAppAnalytics(events = [], pageViewEvents = []) {
  const clickEvents = events
    .filter((event) => WHATSAPP_EVENT_TYPES.has(event.type))
    .map((event) => attributeEvent(event));
  const uniqueClickVisitors = unique(clickEvents, "visitorId");
  const eventRows = clickEvents
    .slice()
    ...
    .map((event) => ({
      ...
      placement: event.whatsapp?.placement || "other",
      component: event.whatsapp?.component || "site-link",
      productSlug: event.whatsapp?.productSlug || "",
      productName: event.whatsapp?.productName || "",
      locale: event.whatsapp?.locale || "",
  ...
  return {
    overview: {
      ...
      uniqueVisitors: uniqueClickVisitors,
      sessions: unique(clickEvents, "sessionId"),
      clickThroughRate: pageViewEvents.length
        ? Number(((clickEvents.length / pageViewEvents.length) * 100).toFixed(2))
        : 0,
      floatingClicks: clickEvents.filter((event) => event.whatsapp?.placement === "floating").length
    },
    placements: countBySelector(clickEvents, (event) => event.whatsapp?.placement || "other", 12),
    pages: countBySelector(clickEvents, (event) => event.page || "/", 12),
    products: countBySelector(clickEvents.filter((event) => event.whatsapp?.productName), (event) => event.whatsapp.productName, 12),
    countries: countBy(clickEvents, "country", 10),
    devices: countBy(clickEvents, "device", 6),
    channels: countBy(clickEvents, "channel", 8),
    sourcePlatforms: countBy(clickEvents, "sourcePlatform", 10),
    events: eventRows
  };
}
...
async function readStoredSnapshot(cacheKey, { allowStale = true } = {}) {
  if (!isDatabaseConfigured()) return null;
  try {
    const stored = await readDatabaseAnalyticsSnapshot({
      cacheKey,
      maxAgeMs: allowStale ? PERSISTENT_SNAPSHOT_MAX_STALE_MS : PERSISTENT_SNAPSHOT_TTL_MS,
      allowStale
    });
    ...
    return {
      ...stored.snapshot,
      cache: {
        status: stored.isFresh ? "persistent-hit" : "persistent-stale",
        generatedAt: stored.snapshot.generatedAt || stored.generatedAt,
        ttlMs: PERSISTENT_SNAPSHOT_TTL_MS,
        ageMs: stored.ageMs,
        eventCount: stored.eventCount
      }
    };
  } catch (error) {
    console.error("Database analytics snapshot read failed", error);
    lastAnalyticsDatabaseError = publicDatabaseError(error);
    return null;
  }
}

async function writeStoredSnapshot({ cacheKey, options, startDate, endDate, dashboard, eventCount }) {
  if (!isDatabaseConfigured() || options.store === false || options.includeSearchConsole === true) return;
  try {
    await writeDatabaseAnalyticsSnapshot({
      cacheKey,
    ...
    console.error("Database analytics snapshot write failed", error);
    lastAnalyticsDatabaseError = publicDatabaseError(error);
  }
}

async function computeAnalyticsSnapshot({ options, startDate, endDate, rangeDays, includeSearchConsole, useCache, cacheKey }) {
  const currentEvents = await readAnalyticsEvents({
    startDate,
    endDate,
    days: Math.min(731, rangeDays + 3),
    limit: ANALYTICS_SNAPSHOT_EVENT_LIMIT
  });
  const currentVisitorIds = [...new Set(currentEvents.map((event) => event?.visitorId).filter(Boolean).map(String))];
  const historyEvents = await readVisitorHistoryEvents({
    days: 731,
    visitorIds: currentVisitorIds,
    limit: Math.min(100000, Math.max(1000, currentVisitorIds.length * 60))
  });
  ...
  const customerProfiles = buildCustomerProfiles(attributedPageViewEvents, visitorDayProfiles, visitorNumbers);
  ...
  const dashboard = {
    ...
    rangeStart: startDate.toISOString(),
    rangeEnd: endDate.toISOString(),
    storageMode: lastAnalyticsDatabaseError ? "database-unavailable-file-fallback" : getAnalyticsStorageMode(),
    databaseStatus: lastAnalyticsDatabaseError ? "unavailable" : isDatabaseConfigured() ? "ok" : "not-configured",
    databaseError: lastAnalyticsDatabaseError,
    ...
    customerProfiles,
    pages,
    whatsapp: buildWhatsAppAnalytics(events, pageViewEvents),
  ...
  await writeStoredSnapshot({ cacheKey, options, startDate, endDate, dashboard, eventCount: currentEvents.length });

  return dashboard;
}

export async function readAnalyticsVisitorJourney({ visitorId = "", sessionId = "", clickedAt } = {}) {
  const safeVisitorId = String(visitorId || "").trim().slice(0, 80);
  const safeSessionId = String(sessionId || "").trim().slice(0, 80);
  if (!safeVisitorId && !safeSessionId) return { available: true, events: [] };

  if (isDatabaseConfigured()) {
    try {
      const events = await readDatabaseVisitorJourney({
        visitorId: safeVisitorId,
        sessionId: safeSessionId,
        submittedAt: clickedAt || new Date().toISOString(),
        beforeDays: 365,
        afterDays: 1,
        limit: 120
      });
      return {
        available: true,
        events: (events || []).filter(isCustomerJourneyEvent)
      };
    } catch (error) {
      lastAnalyticsDatabaseError = publicDatabaseError(error);
  ...
  if (!canUseFileAnalytics()) return { available: false, events: [] };
  const events = await readAnalyticsEvents({ days: 365, limit: ANALYTICS_SNAPSHOT_EVENT_LIMIT });
  return {
    available: true,
    events: events
      .filter((event) => (safeVisitorId && event.visitorId === safeVisitorId) || (safeSessionId && event.sessionId === safeSessionId))
      .filter(isCustomerJourneyEvent)
      .map(cleanStoredEvent)
      .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
      .slice(0, 120)
  };
}
...
export async function getAnalyticsSnapshot(options = {}) {
  ...
  if (useCache && options.preferStoredSnapshot !== false && includeSearchConsole !== true) {
    const stored = await readStoredSnapshot(cacheKey, { allowStale: true });
    if (stored) {
      snapshotCache.set(cacheKey, { createdAt: Date.now(), value: stored });
      return stored;
    }
  }
...
export async function refreshStoredAnalyticsSnapshots(ranges = []) {
  const results = [];
  for (const range of ranges) {
    ...
    try {
      ...
      results.push({
        ...
        pageViews: snapshot.overview?.pageViews || 0,
      ...
      results.push({
        ...
        status: "failed",
        durationMs: Date.now() - started,
        error: publicDatabaseError(error) || "snapshot refresh failed"
      });
    }
  }
  return results;
...
export async function countAnalyticsEvents(options = {}) {
  if (isDatabaseConfigured()) {
    try {
      const count = await countDatabaseAnalyticsEvents(options);
      lastAnalyticsDatabaseError = "";
      return count ?? 0;
    } catch (error) {
      console.error("Database analytics count failed", error);
      lastAnalyticsDatabaseError = publicDatabaseError(error);
    }
  }

  const events = await readAnalyticsEvents({ ...options, limit: ANALYTICS_SNAPSHOT_EVENT_LIMIT });
  return events.length;
}

// Health checks must not materialize the full admin dashboard. That work includes
// visitor-history and attribution queries which are useful in the dashboard but
// can turn a simple liveness probe into an avoidable timeout.
export async function getAnalyticsHealth() {
  const eventCount = await countAnalyticsEvents({ days: 1 });
  return {
    storageMode: lastAnalyticsDatabaseError ? "database-unavailable-file-fallback" : getAnalyticsStorageMode(),
    databaseStatus: lastAnalyticsDatabaseError ? "unavailable" : isDatabaseConfigured() ? "ok" : "not-configured",
    databaseError: lastAnalyticsDatabaseError,
