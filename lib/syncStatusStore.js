...
async function ensureSchema() {
  if (!getPools() || schemaReady) return;
...
export async function withSyncJobLock(jobName, callback) {
  if (!getPools()) return callback({ locked: true, storageMode: "file" });

  await ensureSchema();
  const connection = await withDatabaseFallback(async (db) => ({ client: await db.connect() }));
...
export async function getLatestSyncJobRuns(jobName = "analytics-sync", limit = 10) {
  if (getPools()) {
    await ensureSchema();
    const result = await withDatabaseFallback((db) => db.query(
      `
...
export async function getSyncStatus(jobName = "analytics-sync") {
  ...
  const schedules = {
    "analytics-sync": "*/30 * * * *; analytics snapshot refresh only",
    "blog-webhook-retry": "*/30 * * * *; retries failed external Blog publications up to three times",
    "monthly-inquiry-test": "15 1 1 * *; monthly inquiry delivery test",
    "sitemap-maintenance": "35 2 * * *; daily sitemap consistency check and Search Console submission only when the sitemap changes"
  };
  return {
    jobName,
