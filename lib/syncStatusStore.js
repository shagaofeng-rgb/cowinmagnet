...
export async function getSyncStatus(jobName = "analytics-sync") {
  ...
  const schedules = {
    ...
    "blog-automation": "disabled; automatic Blog publishing is paused",
    "news-automation": "handled by analytics-sync every 3 hours; max 1 published post per run",
    "monthly-inquiry-test": "manual route only; not configured as a Vercel Cron job",
    "sitemap-maintenance": "manual route only; sitemap is refreshed from publish/update paths"
  };
  return {
    jobName,
