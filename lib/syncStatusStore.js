...
export async function getSyncStatus(jobName = "analytics-sync") {
  ...
  const schedules = {
    "analytics-sync": "*/30 * * * *",
    "news-automation": "0 */3 * * *; publishes only when a daily random window is due",
    "monthly-inquiry-test": "0 1 1 * *; sends one inquiry email delivery test on the first day of each month"
  };
  return {
    jobName,
