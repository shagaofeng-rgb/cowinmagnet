...
export async function getSyncStatus(jobName = "analytics-sync") {
  ...
  const schedules = {
    ...
    "news-automation": "manual/admin endpoint only; not registered as a Vercel Cron job",
    "monthly-inquiry-test": "manual route only; not registered as a Vercel Cron job"
  };
  return {
    jobName,
