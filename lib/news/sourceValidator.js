...
// This deliberately checks only a small batch on every run.  A public news page is a
// valid discovery endpoint; RSS is optional.  Robots.txt remains a hard runtime boundary.
export async function runNewsSourceHealthCheck({ siteId, fetcher = fetch, limit = 3 } = {}) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      await updateNewsSourceValidation({ siteId, domain: source.domain, validationStatus: "enabled-public-page", robotsAllowed: true, active: true, notes: "Public page and robots are reachable; enabled for low-frequency public-page discovery.", retryAfterMinutes: 10_080 });
      results.push({ domain: source.domain, status: "enabled-public-page", httpStatus: page.status });
    } catch (error) {
