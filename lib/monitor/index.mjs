...
export function monitoredImageUrl(siteUrl, tag) {
  const srcset = attr(tag, "srcset") || attr(tag, "data-srcset");
  if (srcset) {
    const candidates = srcset
      .split(",")
      .map((item) => item.trim().match(/^(\S+)\s+(\d+)w$/))
      .filter(Boolean)
      .map((match) => ({ url: match[1], width: Number(match[2]) }))
      .sort((left, right) => left.width - right.width);
    const desktopCandidates = candidates.filter((candidate) => candidate.width <= 1200);
    const selected = desktopCandidates[desktopCandidates.length - 1] || candidates[0];
    if (selected?.url) return absoluteUrl(siteUrl, selected.url);
  }
  const src = attr(tag, "src") || attr(tag, "data-src");
  return src ? absoluteUrl(siteUrl, src) : "";
}
...
function extractResources(siteUrl, html) {
  ...
  for (const tag of imgTags(html)) {
    ...
    const src = monitoredImageUrl(siteUrl, tag);
    if (src && !src.startsWith("data:")) resources.push({ type: "image", url: src });
  }
...
async function fetchWithTiming(url, { timeoutMs, method = "GET", headers = {} }) {
...
export function isTransientFetchFailure(response) {
  return response.status === 0 || response.status >= 500;
}

async function fetchWithRetry(url, options) {
  let response = await fetchWithTiming(url, options);
  if (!isTransientFetchFailure(response)) return response;
  await new Promise((resolve) => setTimeout(resolve, 180));
  response = await fetchWithTiming(url, options);
  return response;
}
...
async function discoverSitemapPages(config, issues) {
  ...
  while (queue.length > 0 && urls.length < config.maxSitemapUrls) {
    ...
    const response = await fetchWithRetry(currentUrl, { timeoutMs: config.timeoutMs });
    if (!response.ok) {
      issues.push(
        issue(
...
async function checkResource(resource, pageUrl, config, resourceFetchCache) {
  let resourcePromise = resourceFetchCache.get(resource.url);
  if (!resourcePromise) {
    resourcePromise = (async () => {
      const headers = resource.type === "image" ? { Accept: "image/avif,image/webp,image/apng,image/*,*/*;q=0.8" } : {};
      let response = await fetchWithRetry(resource.url, { timeoutMs: config.timeoutMs, method: "HEAD", headers });
      if (!response.ok && response.status === 405) {
        response = await fetchWithRetry(resource.url, { timeoutMs: config.timeoutMs, method: "GET", headers });
      }
      return response;
    })();
    resourceFetchCache.set(resource.url, resourcePromise);
  }
...
function checkInteractions({ url, html }) {
  ...
  if (/\/contact|\/request-quote/.test(new URL(url).pathname)) {
    ...
    if (!hrefs.some((href) => href.startsWith("tel:") || /^(https?:)?\/\/wa\.me\//i.test(href))) {
      issues.push(issue("P2", "interaction", url, "Contact page has no phone or WhatsApp contact link.", "Add a direct phone or WhatsApp contact link."));
    }
...
async function checkPage(page, config, resourceFetchCache) {
  const url = absoluteUrl(config.siteUrl, page.path);
  const response = await fetchWithRetry(url, { timeoutMs: config.timeoutMs });
  const issues = [];
  ...
  const resourceResults = await mapLimit(resources, 6, (resource) => checkResource(resource, url, config, resourceFetchCache));
...
export async function runWebsiteMonitor(options = {}) {
  ...
  const [pageChecks, apiChecks] = await Promise.all([
    mapLimit(pages, 4, (page) => checkPage(page, config, resourceFetchCache)),
