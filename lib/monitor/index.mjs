...
async function discoverSitemapPages(config, issues) {
  ...
  const response = await fetchWithTiming(sitemapUrl, { timeoutMs: config.timeoutMs });
  if (!response.ok) {
    issues.push(issue("P1", "seo", sitemapUrl, `sitemap.xml is not reachable: ${response.status || response.error}.`, "Confirm sitemap generation and robots settings."));
    return [];
  }

  const urls = [...response.text.matchAll(/<loc>([\s\S]*?)<\/loc>/gi)]
...
function checkSeo({ url, html }) {
  const title = titleOf(html);
  const description = metaContent(html, "description");
  const canonical = canonicalOf(html);
  const h1s = h1Count(html);
  const images = imgTags(html);
  ...
  if (/\/(en|es|ru|ar|fr|pt)(\/|$)/.test(new URL(url).pathname) && hreflangs === 0) {
    issues.push(issue("P2", "seo", url, "Localized page has no hreflang alternates.", "Add alternates.languages metadata."));
  }
...
async function checkPage(page, config) {
  ...
  if (response.ok && visibleText.length < 120) {
  ...
  const seo = response.ok ? checkSeo({ url, html }) : null;
  if (seo) issues.push(...seo.issues);

  const interaction = response.ok ? checkInteractions({ url, html }) : null;
  if (interaction) issues.push(...interaction.issues);

  const resources = response.ok ? extractResources(config.siteUrl, html).slice(0, config.maxResourcesPerPage) : [];
...
function addDuplicateSeoIssues(pageChecks, issues) {
  ...
  for (const page of pageChecks) {
    ...
    if (page.seo.title) byTitle.set(page.seo.title, [...(byTitle.get(page.seo.title) || []), page.url]);
    if (page.seo.description) byDescription.set(page.seo.description, [...(byDescription.get(page.seo.description) || []), page.url]);
  }
