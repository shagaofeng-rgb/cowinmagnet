...
import { includesNormalizedNewsTerm, newsFingerprint, normalizeNewsText, parseNewsRssItems, sourceMatchesAllowedHost } from "./newsOperationsRules.js";
...
function scoreCandidate({ source, item, site, family, now }) {
  ...
  const relevanceMatches = (site.relevance_terms || []).filter((term) => includesNormalizedNewsTerm(normalizedBody, term));
...
function sourceMatchesWhitelist(item, source) {
  return sourceMatchesAllowedHost(item.sourceUrl, source.domain, source.rss_or_api_url);
}
