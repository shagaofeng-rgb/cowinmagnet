...
function newsComposeTimeoutMs() {
  ...
  // Leave enough of the 120-second serverless budget for validation and delivery checks.
  return Math.max(10000, Math.min(35000, Number.isFinite(configured) ? configured : 28000));
}
...
async function fetchNewsWithTimeout(fetcher, url, options = {}, timeoutMs, label) {
...
async function composeNewsArticle({ site, candidate, fetcher = fetch, productTheme = null }) {
  ...
  const response = await fetchNewsWithTimeout(fetcher, "https://api.openai.com/v1/responses", {
    method: "POST",
  ...
  if (!document?.title || !document?.seo?.metaTitle || !document?.seo?.metaDescription || !article.slug) throw new Error("Composer returned incomplete structured News JSON");
  ...
  if (!document.sources?.[0]?.url || normalizedUrl(document.sources[0].url) !== candidate.canonicalUrl) throw new Error("Composer source panel does not match the selected candidate");
...
export async function verifyFrontendNewsPublication({ site, slug, title, contentType = "news", fetcher = fetch }) {
  ...
  const [list, detail, sitemap, rss, blog] = await Promise.all([
    fetchNewsWithTimeout(fetcher, listUrl, {}, timeoutMs, "News list verification"),
    fetchNewsWithTimeout(fetcher, detailUrl, {}, timeoutMs, "News detail verification"),
    fetchNewsWithTimeout(fetcher, sitemapUrl, {}, timeoutMs, "News sitemap verification"),
    fetchNewsWithTimeout(fetcher, rssUrl, {}, timeoutMs, "News RSS verification"),
    fetchNewsWithTimeout(fetcher, blogUrl, {}, timeoutMs, "Blog isolation verification")
  ]);
  ...
  const checks = {
    ...
    schemaValid: detail.ok && /application\/ld\+json/i.test(detailHtml) && new RegExp(contentType === "news" ? "NewsArticle" : "Article", "i").test(detailHtml),
    sourcePanelVisible: contentType !== "news" || (detail.ok && /<h2>Sources<\/h2>/i.test(detailHtml)),
