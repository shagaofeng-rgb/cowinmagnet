...
function fitMetadata(value, maxLength) {
  const normalized = String(value || "").replace(/\s+/g, " ").trim();
  if (normalized.length <= maxLength) return normalized;
  const clipped = normalized.slice(0, maxLength + 1);
  const wordBoundary = clipped.lastIndexOf(" ");
  return clipped.slice(0, wordBoundary >= Math.floor(maxLength * 0.7) ? wordBoundary : maxLength).replace(/[,:;\-]+$/, "").trim();
}
...
async function generateWithOpenAI({ plan, product, candidates, fetcher = fetch }) {
  ...
  generated.metaDescription = fitMetadata(generated.metaDescription, 160);
...
export async function runNewsPublishCycle() {
  return withNewsPublicationLock(async () => {
    ...
    try {
      ...
      if (!plan) {
        const result = { status: "skipped", reason: "No balanced planned topic currently has two independent eligible sources" };
        await finishNewsPublicationRun(run, { status: "skipped", logs: [result] });
        return result;
      }
