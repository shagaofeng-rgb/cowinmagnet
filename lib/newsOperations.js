...
async function composeNewsArticle({ site, candidate, fetcher = fetch, productTheme = null }) {
  ...
  const timeout = setTimeout(() => controller.abort(), newsComposeTimeoutMs());
  let response;
  try {
    ...
    if (controller.signal.aborted) throw new Error(`News composer timed out after ${newsComposeTimeoutMs()}ms`);
    throw error;
  } finally {
