...
async function composeNewsArticle({ site, candidate, fetcher = fetch, productTheme = null, publicationContext = null }) {
  if (!process.env.OPENAI_API_KEY) {
    const fallback = createSourceBoundFallbackNewsDocument({ site, candidate, productTheme, publicationContext });
  ...
  return { slug: article.slug, document: publicationContext ? bindPublicationContext(document, publicationContext) : document };
}
