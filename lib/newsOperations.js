...
function newsPublishCandidateLimit() {
  ...
  // A single function invocation must not serially exhaust its serverless budget.
  return Math.max(1, Math.min(2, Number.isFinite(configured) ? configured : 2));
}
...
function generationPrompt({ site, candidate, productTheme }) {
  ...
  return `Write one English industry news editorial as strict JSON for a configured B2B site. It is an independent, concise summary and analysis of the cited source, not a rewrite, translation, promotion, press release or sales page. Use only supplied source facts. Do not invent numbers, claims, quotes, authors, companies, certifications, images or dates. Do not include contact details, quotations, sales language, brand claims, product cards or more than one optional internal URL. Return exactly {"slug":"","document":{"schemaVersion":1,"locale":"en","contentType":"news","status":"draft","title":"","summary":"","primaryTopic":"","targetAudience":"","sections":[{"heading":"","level":2,"blocks":[{"type":"paragraph","text":""}]}],"faq":[],"sources":[{"title":"","publisher":"","url":"","publishedAt":"","accessedAt":"","relevanceNote":""}],"relatedContent":[],"cta":{"heading":"Explore related equipment","text":"Review related equipment categories and selection information for industrial material-handling applications.","label":"Explore related equipment","href":"/en/products"},"seo":{"metaTitle":"","metaDescription":"","canonicalPath":"","ogTitle":"","ogDescription":""},"author":{"name":"${site.brand_name} Editorial Team","profilePath":"/editorial-policy","role":"Editorial Team"}}}. Use 5 to 8 section headings total including FAQ. Use only paragraph, bullets, numbered-list, checklist or callout blocks; list items must never become headings. The full title, summary, sections and FAQ content together must be 760-880 English words. Make metaDescription a complete 120-150 character sentence. Sections should explain News facts, What changed, Why this matters, Editorial perspective and practical buyer questions. Distinguish sourced facts from editorial analysis. Source facts: ${JSON.stringify({ publisher: candidate.publisher, title: candidate.title, url: candidate.canonicalUrl, publishedAt: candidate.sourcePublishedAt, author: candidate.author || null, summary: facts.summary || "" })}. Site industry scope: ${site.industry_scope}. Current optional product theme: ${productTheme ? JSON.stringify(productTheme) : "none; omit relatedContent"}.`;
}
...
function sourceFact(candidate) {
  return String(candidate.facts?.[0]?.summary || candidate.title || "The source describes a development relevant to industrial material handling.")
    .replace(/\s+/g, " ")
    .trim();
}
...
export function createSourceBoundFallbackNewsDocument({ site, candidate, productTheme = null }) {
  ...
  const document = {
    ...
    sections: [
      ...
      {
        heading: "What to monitor next",
        level: 2,
        blocks: [{ type: "paragraph", text: `Monitor follow-up information from the original publisher and other authoritative industry sources, especially where the update changes specifications, handling practices, regulations or supply conditions. Keep the original publication date in view when discussing the item internally. If the topic affects an active project, record the actual material, capacity, installation constraints and required outcome so that any later technical discussion is tied to evidence rather than a general headline.` }]
      }
    ],
    faq: [
...
async function composeNewsArticle({ site, candidate, fetcher = fetch, productTheme = null }) {
  ...
  const payload = await withNewsTimeout(async (signal) => {
    const response = await fetcher("https://api.openai.com/v1/responses", {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${process.env.OPENAI_API_KEY}` },
      body: JSON.stringify({
        model: process.env.NEWS_LLM_MODEL || "gpt-5-mini",
        input: [{ role: "user", content: generationPrompt({ site, candidate, productTheme }) }],
  ...
  try {
    ...
    return createSourceBoundFallbackNewsDocument({ site, candidate, productTheme });
  }
  ...
  if (!document?.title || !document?.seo?.metaTitle || !document?.seo?.metaDescription || !article.slug || words < site.news.desired_word_count.min || words > site.news.desired_word_count.max || !document.sources?.[0]?.url || normalizedUrl(document.sources[0].url) !== candidate.canonicalUrl) {
    return createSourceBoundFallbackNewsDocument({ site, candidate, productTheme });
  }
  return { slug: article.slug, document };
}
      ...
      for (const candidate of candidates.slice(0, newsPublishCandidateLimit())) {
        ...
        try {
          ...
          await saveCmsItem({
            ...
            coverImage: document.heroImage?.assetId || "", coverAlt: document.heroImage?.alt || "", imageCaption: document.heroImage?.caption || "", bodyImages: [], relatedProducts: [], faqs: document.faq, tags: [reserved.industry].filter(Boolean), publishedAt: document.publishedAt, status: "published", href: `/news/${article.slug}`
          });
