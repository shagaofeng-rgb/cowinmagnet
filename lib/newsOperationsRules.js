...
export function includesNormalizedNewsTerm(value = "", term = "") {
  const haystack = ` ${normalizeNewsText(value)} `;
  const needle = normalizeNewsText(term);
  return Boolean(needle) && haystack.includes(` ${needle} `);
}

export function sourceMatchesAllowedHost(itemSourceUrl, sourceDomain, discoveryUrl = null) {
  try {
    const itemHost = new URL(itemSourceUrl).hostname.replace(/^www\./, "");
    const allowedHosts = [sourceDomain, discoveryUrl ? new URL(discoveryUrl).hostname : null]
      .filter(Boolean)
      .map((host) => host.replace(/^www\./, ""));
    return allowedHosts.includes(itemHost);
  } catch {
    return false;
  }
}
...
export function normalizeGeneratedSourceClaims(claims = [], candidates = []) {
  if (!Array.isArray(claims)) return [];
  return claims.map((claim) => {
    const rawClaim = typeof claim === "string" ? { sourceUrl: claim } : (claim || {});
    const sourceUrl = rawClaim.sourceUrl || rawClaim.source_url || rawClaim.url || rawClaim.link || "";
    const candidate = candidates.find((item) => (item.source_url || item.sourceUrl) === sourceUrl);
    if (!candidate) return null;
    return {
      ...rawClaim,
      sourceUrl,
      candidateId: candidate.id || null,
      publisher: candidate.publisher || rawClaim.publisher || "External source",
      publishedAt: candidate.published_at || candidate.publishedAt || rawClaim.publishedAt || null,
      sourceTitle: rawClaim.sourceTitle || candidate.title || rawClaim.title || "Source reference",
      usageNote: rawClaim.usageNote || "Contextual fact source",
    };
  }).filter(Boolean);
}
