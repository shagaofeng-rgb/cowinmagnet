...
const FORBIDDEN_CLAIMS = /\b(FDA|HACCP|ATEX|CE certified|food[- ]grade|sterile|guaranteed|best|leading|100%|payback|ROI|recovery rate)\b/i;
...
export function validateNewsArticle(article, { catalog, knownArticleFingerprints = [] } = {}) {
  ...
  if (!Array.isArray(article.mediaPlan) || !article.mediaPlan.some((media) => media?.kind === "licensed-industry-image" || media?.kind === "self-made-process-diagram")) errors.push("requires-licensed-industry-media-or-self-made-diagram");
