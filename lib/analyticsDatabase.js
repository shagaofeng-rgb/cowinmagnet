...
function getPool() {
  ...
  if (!pools) {
    pools = databaseUrlFallbacks().map((connectionString) => new Pool({
      connectionString,
      ssl: databaseSsl(),
      max: databasePoolMax(),
      connectionTimeoutMillis: Math.min(Number(process.env.ANALYTICS_DB_CONNECTION_TIMEOUT_MS || 5000), 3500),
  ...
  if (!resilientPool) {
    resilientPool = {
      query: (text, values) => withDatabaseFallback((db) => db.query(text, values)),
      connect: () => withDatabaseFallback((db) => db.connect())
    };
  }
  return resilientPool;
}

async function withDatabaseFallback(operation) {
  const candidates = pools || [];
  let lastError;

  for (const db of candidates) {
    try {
      return await withDatabaseRetry(() => operation(db), { attempts: 2, delayMs: 250 });
    } catch (error) {
      lastError = error;
  ...
  throw lastError || new Error("No database connection is configured");
}
...
async function ensureSchema() {
  ...
  schemaPromise = (async () => {
    ...
    if (existing.rows[0]?.ready) {
      await db.query("CREATE INDEX IF NOT EXISTS analytics_events_whatsapp_placement_time_idx ON analytics_events ((payload->'whatsapp'->>'placement'), event_time DESC)");
      schemaReady = true;
      return;
    }
    ...
    await client.query("CREATE INDEX IF NOT EXISTS analytics_events_whatsapp_placement_time_idx ON analytics_events ((payload->'whatsapp'->>'placement'), event_time DESC)");
    await client.query(`
...
export async function readDatabaseEvents({ days = 90, startDate, endDate, limit = 8000 } = {}) {
  ...
  const eventFields = `
    SELECT
      ...
      COALESCE((payload->>'isTest')::BOOLEAN, FALSE) AS is_test,
      COALESCE((payload->>'operatorTraffic')::BOOLEAN, FALSE) AS operator_traffic
  ...
  const toEvent = (row) => ({
    ...
    operatorTraffic: Boolean(row.operator_traffic)
  });
