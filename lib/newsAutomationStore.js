...
function getResilientPool() {
  const database = getPool();
  if (!database) return null;
  if (!resilientPool) {
    resilientPool = {
      query: (text, values) => withDatabaseRetry(() => database.query(text, values), { attempts: 2, delayMs: 250 })
    };
  }
...
export async function listNewsSources(siteId) {
  const connection = await db();
  // Rotation is a preference, not a hard cooldown: a fourteen-day exclusion cannot
  // sustain daily publishing when the verified source pool is smaller than fourteen.
  const result = await connection.query("SELECT * FROM news_sources WHERE site_id=$1 AND allowed=TRUE AND active=TRUE AND validation_status NOT IN ('inactive','robots-blocked') AND tier <> 'discovery-only' AND canonical_duplicate_of IS NULL ORDER BY (rss_url IS NULL), last_used_at NULLS FIRST, source_trust_score DESC, domain", [siteId]);
  return result.rows;
}
...
export async function findCandidateFingerprint({ siteId, fingerprint, fingerprintType, excludeSourceUrl = null }) {
  const connection = await db();
  const result = await connection.query(
    `SELECT fingerprint.candidate_id
     FROM news_candidate_fingerprints fingerprint
     JOIN news_candidates candidate ON candidate.id=fingerprint.candidate_id
     WHERE fingerprint.site_id=$1
       AND fingerprint.fingerprint=$2
       AND fingerprint.fingerprint_type=$3
       AND ($4::text IS NULL OR candidate.source_url<>$4)
     LIMIT 1`,
    [siteId, fingerprint, fingerprintType, excludeSourceUrl]
  );
...
export async function getSuccessfulNewsPublicationForDay({ siteId, timeZone, publicationDate }) {
  const connection = await db();
  const result = await connection.query(
    `SELECT * FROM news_publication_runs
     WHERE site_id=$1
       AND run_type IN ('daily-publish','48-hour-publish')
       AND status='published_success'
       AND (finished_at AT TIME ZONE $2)::date = $3::date
     ORDER BY finished_at DESC
     LIMIT 1`,
    [siteId, timeZone, publicationDate]
  );
  return result.rows[0] || null;
}
...
export async function rejectNewsCandidate({ siteId, candidateId, reason }) {
  const connection = await db();
  await connection.query("UPDATE news_candidates SET status='rejected',reserved_at=NULL,rejection_reason=$3,updated_at=NOW() WHERE id=$1 AND site_id=$2 AND status='reserved_for_cycle'", [candidateId, siteId, reason || "publication-source-unavailable"]);
}
...
export async function getNewsAutomationDashboard(siteId = getNewsSiteConfig().site_id) {
  ...
  const today = publicationDateKey(new Date(), site.timezone);
  const [sources, candidates, runs, lastSuccess, todaySuccess] = await Promise.all([
    listNewsSources(siteId),
    listNewsCandidates({ siteId, limit: 20 }),
    listNewsRuns({ siteId, limit: 20 }),
    getLastSuccessfulNewsPublication(siteId),
    getSuccessfulNewsPublicationForDay({ siteId, timeZone: site.timezone, publicationDate: today })
  ]);
  const latestPublishRun = runs.find((run) => ["daily-publish", "48-hour-publish"].includes(run.run_type)) || null;
  return {
    siteId,
    storageMode: newsAutomationStorageMode(),
    sources,
    candidates,
    runs,
    publicationStatus: {
      ...
      today,
      publishedToday: Boolean(todaySuccess),
      lastSuccessfulAt: lastSuccess?.finished_at || null,
      latestRunStatus: latestPublishRun?.status || null,
      latestRunError: latestPublishRun?.error_summary || null,
      eligibleCandidateCount: candidates.filter((candidate) => candidate.status === "candidate").length
    }
  };
}
