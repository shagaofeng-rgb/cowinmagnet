...
export async function recoverStaleNewsPublishWork({ siteId, maxAgeMinutes = 10 } = {}) {
  ...
  const [runs, candidates] = await Promise.all([
    connection.query(
      "UPDATE news_publication_runs SET status='retry_pending',finished_at=NOW(),error_summary=$3,logs_json=logs_json || $4::jsonb WHERE site_id=$1 AND run_type='48-hour-publish' AND status='running' AND started_at < NOW() - ($2 * INTERVAL '1 minute') RETURNING id",
