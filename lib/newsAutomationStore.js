...
export async function syncNewsSources(site) {
  ...
  for (const source of sources) {
    await connection.query(
      `INSERT INTO news_sources (id, site_id, domain, name, priority, rss_url, allowed, active, source_type, source_trust_score, allowed_topics, allowed_languages, source_ordinal, raw_entry, requested_url, source_group, discovery_methods, tier, validation_status, robots_allowed, canonical_duplicate_of, notes)
       ...
       validation_status=CASE WHEN EXCLUDED.validation_status='verified' THEN 'verified' ELSE news_sources.validation_status END,
       robots_allowed=CASE WHEN EXCLUDED.robots_allowed IS TRUE THEN TRUE ELSE news_sources.robots_allowed END,
       active=CASE WHEN EXCLUDED.validation_status='verified' THEN TRUE WHEN EXCLUDED.validation_status IN ('pending','needs_review','inactive','robots-blocked') THEN FALSE ELSE news_sources.active END, canonical_duplicate_of=COALESCE(EXCLUDED.canonical_duplicate_of, news_sources.canonical_duplicate_of), notes=COALESCE(EXCLUDED.notes, news_sources.notes), updated_at=NOW()`,
  ...
  // A legacy configuration must not leave an unverified source switchable on.
  // This is intentionally a state correction only; rows and their raw import are retained.
  await connection.query("UPDATE news_sources SET active=FALSE, updated_at=NOW() WHERE site_id=$1 AND validation_status <> 'verified' AND active=TRUE", [site.site_id]);
}

export async function listNewsSources(siteId) {
  const connection = await db();
  const result = await connection.query("SELECT * FROM news_sources WHERE site_id=$1 AND allowed=TRUE AND active=TRUE AND validation_status='verified' AND robots_allowed=TRUE AND tier <> 'discovery-only' AND rss_url IS NOT NULL AND (last_used_at IS NULL OR last_used_at < NOW() - INTERVAL '14 days') ORDER BY last_used_at NULLS FIRST, source_trust_score DESC, domain", [siteId]);
  return result.rows;
}
