...
export async function getLiveSearchConsoleSnapshot({ days = 28 } = {}) {
  ...
  const [overviewData, queryData, pageData, countryData, deviceData] = await Promise.all([
    querySearchAnalytics(accessToken, { startDate, endDate, rowLimit: 1 }),
    querySearchAnalytics(accessToken, { startDate, endDate, dimensions: ["query"], rowLimit: 20 }),
    querySearchAnalytics(accessToken, { startDate, endDate, dimensions: ["page"], rowLimit: 20 }),
    querySearchAnalytics(accessToken, { startDate, endDate, dimensions: ["country"], rowLimit: 12 }),
    querySearchAnalytics(accessToken, { startDate, endDate, dimensions: ["device"], rowLimit: 8 })
  ]);
