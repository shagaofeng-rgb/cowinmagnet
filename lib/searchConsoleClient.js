...
function sitemapStatusShape({ data = {}, live = false, error = "" } = {}) {
  ...
  return {
    ...
    submissionEnabled: isSearchConsoleSubmissionEnabled(),
    live,
    siteUrl: process.env.GOOGLE_SEARCH_CONSOLE_SITE_URL || "",
...
// The Search Console Sitemaps API is the authoritative, supported way to
// verify that Google has accepted and read our sitemap. It deliberately does
// not claim that individual URLs have been indexed; Page indexing coverage is
// available in the Search Console product, not as a bulk coverage API.
export async function getSearchConsoleSitemapStatus({
  siteUrl = process.env.GOOGLE_SEARCH_CONSOLE_SITE_URL,
  sitemapUrl = configuredSitemapUrl(),
  fetchImpl = globalThis.fetch
} = {}) {
  if (!isSearchConsoleConfigured()) return sitemapStatusShape();
  if (!siteUrl || !sitemapUrl) throw new Error("Search Console site URL or sitemap URL is missing");

  const accessToken = await getAccessToken(readonlyScope, fetchImpl);
  ...
  const response = await fetchGoogle(endpoint, { headers: { Authorization: `Bearer ${accessToken}` } }, fetchImpl);
  if (!response.ok) {
    const body = (await response.text()).slice(0, 500);
    throw new Error(`Search Console sitemap status failed: ${response.status} ${body}`);
  }
  return sitemapStatusShape({ data: await response.json(), live: true });
}
