...
function secret() {
  return (
    ...
    "cowinmagnet-local-admin-secret"
  );
}
...
function adminPasswordVariants(password) {
...
export async function verifyAdminCredentials(email, password) {
  ...
  if (!normalizedEmail || !isConfiguredAdminEmail(normalizedEmail) || !password) {
    return false;
  }

  const passwordVariants = adminPasswordVariants(password);

  for (const passwordVariant of passwordVariants) {
    if (await verifyStoredAdminPassword(normalizedEmail, passwordVariant)) {
      return true;
    }
  }

  if (process.env.ADMIN_PASSWORD_HASH) {
    return passwordVariants.some((passwordVariant) => hashAdminPassword(passwordVariant) === process.env.ADMIN_PASSWORD_HASH);
  }

  if (process.env.ADMIN_PASSWORD) {
    return passwordVariants.includes(process.env.ADMIN_PASSWORD);
  }

  if (process.env.ADMIN_DEFAULT_PASSWORD) {
    return passwordVariants.includes(process.env.ADMIN_DEFAULT_PASSWORD);
  }
