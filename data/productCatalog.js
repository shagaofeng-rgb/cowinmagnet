  ...
  },
  {
    ...
    "products": [
      ...
      },
      {
        ...
        "overview": "The equipment separates magnetic minerals or ferrous particles from dry bulk material by using a magnetic drum. Magnetic material is attracted to the drum surface and discharged separately from non-magnetic material.",
        "features": [],
        "specifications": [
          [
            "Model",
  ...
  },
  {
    ...
    "products": [
      ...
      },
      {
        ...
        "overview": "product such as sugar, grain, tea, etc. They are widely used in field of ceramics, chemicals, pharmacyfood,plastic,rubber,dyestuff,mining,environmental protection,etc.",
        "features": [],
        "specifications": [
          [
            "Model",
...
export async function getProductBySlugAsync(slug) {
  const cmsItems = await getCmsItems({ type: "product", locale: "en" });
  const cmsItem = cmsItems.find((item) => item.slug === slug);
  if (cmsItem) return cmsItem;
  return getProductBySlug(slug);
}

export async function getProductCategoriesAsync() {
  const cmsCategories = await getCmsProductCategories();
  return cmsCategories.length ? cmsCategories : getProductCategories();
}

export async function getAllProductsAsync() {
  const cmsItems = await getCmsItems({ type: "product", locale: "en" });
  return cmsItems.length ? cmsItems : getAllProducts();
}
